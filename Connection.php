...
namespace think\db;
require_once('SwoolePDO.php');
require_once('SwoolePDOStatement.php');

use InvalidArgumentException;
use thinkphp\swoole\SwoolePDO as PDO;
use thinkphp\swoole\SwoolePDOStatement as PDOStatement;
use think\Container;
use think\Db;
...
abstract class Connection
{
    ...
    public function getCursor($sql, $bind = [], $master = false, $model = null, $condition = null, $relation = null)
    {
        ...
        // 预处理
        $PDOStatement = $this->linkID->prepare($sql);
        ...
        // 参数绑定
        if ($procedure) {
            $this->bindParam($PDOStatement, $bind);
        } else {
            $this->bindValue($PDOStatement, $bind);
        }
        ...
        // 返回结果集
        while ($result = $PDOStatement->fetch($this->fetchType)) {
            if ($model) {
    ...
    public function query($sql, $bind = [], $master = false, $pdo = false)
    {
        ...
        try {
            ...
            // 返回结果集
            return $this->getResult($PDOStatement, $pdo, $procedure);
        } catch (\PDOException $e) {
            if ($this->isBreak($e)) {
    ...
    public function execute($sql, $bind = [], Query $query = null)
    {
        ...
        try {
            ...
            $this->numRows = $PDOStatement->rowCount();
     ...
     * @throws BindParamException
     */
    protected function bindValue($PDOStatement, array $bind = [])
    {
        foreach ($bind as $key => $val) {
            // 占位符
            ...
            if (is_array($val)) {
                ...
                $result = $PDOStatement->bindValue($param, $val[0], $val[1]);
            } else {
     ...
     * @throws BindParamException
     */
    protected function bindParam($PDOStatement, $bind)
    {
        foreach ($bind as $key => $val) {
            $param = is_numeric($key) ? $key + 1 : ':' . $key;
            ...
            if (is_array($val)) {
                ...
                $result = call_user_func_array([$PDOStatement, 'bindParam'], $val);
            } else {
                $result = $PDOStatement->bindValue($param, $val);
            }
     ...
     * @return array
     */
    protected function getResult($PDOStatement, $pdo = false, $procedure = false)
    {
        if ($pdo) {
            // 返回PDOStatement对象处理
            return $PDOStatement;
        ...
        if ($procedure) {
            // 存储过程返回结果
            return $this->procedure($PDOStatement);
        }

        $result = $PDOStatement->fetchAll($this->fetchType);
     ...
     * @return array
     */
    protected function procedure($PDOStatement)
    {
        $item = [];
